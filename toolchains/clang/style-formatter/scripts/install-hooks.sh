...
if [[ -f "${DEST_HOOK}" && "${FORCE}" == "false" ]]; then
    ...
    if grep -q "toolchains/clang-style-formatter" "${DEST_HOOK}" 2>/dev/null; then
...
#!/usr/bin/env bash
# Auto-generated by toolchains/clang-style-formatter/scripts/install-hooks.sh
...
# 2. toolchains/clang-source-build bin/ (optional LLVM build)
# 3. legacy bin/ inside formatter dir
# 4. system PATH
CF_BIN=""
for _candidate in     "${SUBMODULE_ROOT}/.venv/Scripts/clang-format.exe"     "${SUBMODULE_ROOT}/.venv/bin/clang-format"     "${SUBMODULE_ROOT}/../toolchains/clang-source-build/bin/windows/clang-format.exe"     "${SUBMODULE_ROOT}/../toolchains/clang-source-build/bin/linux/clang-format"     "${SUBMODULE_ROOT}/bin/windows/clang-format.exe"     "${SUBMODULE_ROOT}/bin/linux/clang-format"; do
