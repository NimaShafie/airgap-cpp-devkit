...
# Author: Nima Shafie
# =============================================================================
# smoke-test.sh — Verify the toolchains/clang-style-formatter submodule is working.
#
# Tests:
...
#
# Usage:
#   bash toolchains/clang-style-formatter/scripts/smoke-test.sh
#
# Exit codes:
#   0 — all tests passed
...
echo "=================================================================="
echo "  smoke-test.sh  —  toolchains/clang-style-formatter"
echo "  Repo : ${REPO_ROOT}"
echo "  Date : $(date)"
echo "=================================================================="
...
for candidate in \
    ...
    "${SUBMODULE_ROOT}/../toolchains/clang-source-build/bin/windows/clang-format.exe" \
    "${SUBMODULE_ROOT}/../toolchains/clang-source-build/bin/linux/clang-format" \
...
if [[ -f "${HOOK}" ]]; then
    ...
    if grep -q "toolchains/clang-style-formatter\|clang-format" "${HOOK}" 2>/dev/null; then
