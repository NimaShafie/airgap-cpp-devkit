...
All binaries are in `prebuilt-binaries/llvm-toolchain/`:

```
clang-linux/
  clang-llvm-22.1.2-linux-x64-slim.tar.xz.part-aa  (50 MB)
  clang-llvm-22.1.2-linux-x64-slim.tar.xz.part-ab  (50 MB)
  clang-llvm-22.1.2-linux-x64-slim.tar.xz.part-ac  (19 MB)
...
bash llvm-toolchain/setup.sh

# Install only clang (Linux only)
bash llvm-toolchain/setup.sh --component clang

# Install only llvm-mingw
bash llvm-toolchain/setup.sh --component mingw

# Custom prefix
bash llvm-toolchain/setup.sh --prefix /opt/tools/llvm
```
...
|------|----------|-------|------------|
| Admin | Linux | `/opt/airgap-cpp-devkit/llvm-toolchain/clang/` | `/opt/airgap-cpp-devkit/llvm-toolchain/llvm-mingw/` |
| User | Linux | `~/.local/share/airgap-cpp-devkit/llvm-toolchain/clang/` | `~/.local/share/.../llvm-mingw/` |
| Admin | Windows | `C:\Program Files\airgap-cpp-devkit\llvm-toolchain\llvm-mingw\` | same |
| User | Windows | `%LOCALAPPDATA%\airgap-cpp-devkit\llvm-toolchain\llvm-mingw\` | same |
...
- **GCC 15.2** (`gcc-linux` module) recommended on Linux for libstdc++ runtime
